...
-- updata history function before in berore out
-- @name : module name
function updatahistory(name)
    if name == nil then
	return
    end
    ...
    history_section = cur:get_all("system", "history")
    history_num = history_section.num
    local find = false              
    if history_section.h1 == name then
	find = true
    end       
    if history_section.h2 == name then
	find = true        
    end                      
    if history_section.h3 == name then
	find = true        
    end
    if history_section.h4 == name then
	find = true
    end
    if history_section.h5 == name then
	find = true
    end
    if history_section.h6 == name then
	find = true
    end
    
    if not find then                                    
	call("echo num = %s > /dev/console" % {history_num})
	if history_num == "6" then                                   
	    local a = 1                                          
	    while(a < 6)                                         
	    do                                                   
		op = "h"..tostring(a)                        
		if op == "h1" then                           
		    cur:set("system","history",op,history_section.h2)
		elseif op == "h2" then                                        
		    cur:set("system","history",op,history_section.h3)
		elseif op == "h3" then                                        
		    cur:set("system","history",op,history_section.h4)
		elseif op == "h4" then                                        
		    cur:set("system","history",op,history_section.h5)
		elseif op == "h5" then                                        
		    cur:set("system","history",op,history_section.h6)
		end                                                           
		a = a + 1                                                     
	    end                                                                   
	    cur:set("system","history","h6",name)                    
	else                                                                          
	    num = tonumber(history_num) + 1                                       
	    op = "h"..tostring(num)                           
	    cur:set("system","history",op,name)          
	    cur:set("system","history","num",tostring(num))
	end                                                                           
	cur:commit("system")                                                     
    else                                                                                  
	call("echo has exist > /dev/console" )                               
    end                             
end
...
function addhistory(name)
	...
	local cur = uci.cursor()
    local num = 1
    hs = cur:get_all("system", "history")
    hn = hs.num
    local find = false
    while(num <= tonumber(hn))
    do
    	local op = "h"..tostring(num)
    	if hs[op] == name then
    		find = true
    		break
    	end
    ...
    if not find then
    	local op = "h"..tostring(tonumber(hn) + 1)
    	local op_num = "h"..tostring(tonumber(hn) + 1).."_num"
    	local op_stack = "h"..tostring(tonumber(hn) + 1).."_stack"
    	cur:set("system","history",op,name)
    	cur:set("system","history",op_num,"1")
    	cur:set("system","history",op_stack,"0")
    	cur:set("system","history","num",tostring(tonumber(hn) + 1))
    else
    	local op = "h"..tostring(num).."_num"
    	local op_num = hs[op]
    	cur:set("system","history",op,tostring(tonumber(op_num) + 1))
    end
...
-- @name : module --
-- @stack : 0/1 --
function stackhistory(name,stack)
	if name == nil then
		return
	end
    ...
    if find then
    	local op = "h"..tostring(num).."_stack"
    	local op_stack = hs[op]
    	
    	cur:set("system","history",op,tostring(stack))
    	cur:save("system")    
    	cur:commit("system")
    end
...
function gethistory()
    ...
    do
    	local op = "h"..tostring(num)
    	local op_stack = "h"..tostring(num).."_stack"
    	local op_num = "h"..tostring(num).."_num"
    	if hs[op_stack] == "1" then
    		table.insert(data_stack,{
    			name = hs[op],
    			level = hs[op_num]
    			})
    	elseif hs[op_stack] == "0" and hs[op_num] ~= "0" then
			table.insert(data_num,{
    			name = hs[op],
    			level = hs[op_num]
    			})
    	end
    ...
    if #data_num > 0 then
    	table.sort(data_num, sortLevelNameAsc)
    end
    
    if #data_stack > 0 then
    	table.sort(data_stack, sortLevelNameAsc)
    end
    
	for idx, value in ipairs(data_stack) do
		--[[
		datas = datas or {}
		local data = {}
		...
		--data["h"..tostring(idx)] = value.name
	end
	
	if #data_stack < 6 and #data_num > 0 then
		for idx, value in ipairs(data_num) do
			if idx + #data_stack > 6 then
				break
			else
			--[[
				datas = datas or {}
				...
				--data["h"..tostring(idx + #data_stack)] = value.name
			end
		end
	end
...
firmware.prompts = {
		[1] = "非法的固件！", -- 固件校验不通过
		[2] = "当前固件版本需要优先升级到%s版本", -- 路由当前版本不满足升级最低版本要求
		[3] = "错误的固件版本，请下载最新固件", -- 不允许降级到当前升级版本
		[4] = "非法操作,固件上传失败",--http请求中没有带有上传的文件
		[5] = "固件不存在，无法进行升级操作！",--没有上传固件，就请求升级[写flash]操作
}
--- Check sop firmware and return the exit code.
-- @param command	The firmware path
-- @return			Number of exit code.
-- 					- 0 OK
-- 					- 1 Imvalid file
-- 					- 2 REQ_VER is unsatisfied
-- 					- 3 DG_VER is unsatisfied
function firmware.check_image(img)
	local rc
	local f
	local prompt
	...
	luci.util.exec(
		string.format(
			". /lib/functions.sh; " ..
			"include /lib/upgrade; " ..
			"platform_check_image %s >/dev/null; " ..
			"echo $? > /tmp/.upgrade", img
		)
	)

	f = io.open("/tmp/.upgrade", "r")
	rc = tonumber(f:read())
	f:close()

	prompt = firmware.prompts[rc]
	if rc == 2 then
		f = io.open("/tmp/.upgrade_req_ver", "r")
		local ver = f:read()
		f:close()
		prompt = string.format(prompt, ver)
	end

	return rc, prompt
end

function firmware.system_upgrade(img)
	fork_exec("sleep 1; killall lighttpd; sleep 1; /sbin/sysupgrade %q" % img)
end
