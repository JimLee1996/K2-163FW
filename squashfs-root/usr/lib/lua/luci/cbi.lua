...
function Map.addmodule(self, name)
	table.insert(self.module, name)
end
...
function Map.parse(self, readinput, ...)
	...
	if self.save then
		...
		if self:submitstate() and ((not self.proceed and self.flow.autoapply) or luci.http.formvalue("cbi.apply")) then
			...
			for i, name in ipairs(self.module) do
				--sys.updatahistory(name)
				sys.addhistory(name)
			end
...
function AbstractValue.parse(self, section, novld)
	...
	elseif fvalue then
		if self.forceempty then
			self:write(section, fvalue)
		end
	else
		if self.rmempty or self.optional then
			--if parse value is nil, do not remove 
		elseif cvalue ~= fvalue and not novld and false then
...
function Flag.parse(self, section)
	...
	if fexists then
		local fvalue = self:formvalue(section)
		if fvalue or (not self.optional and not self.rmempty) then
			self:write(section, fvalue)
		else
			self:write(section, "0")
		end
	else
		self:remove(section)
	end
...
ReadGlass = class(AbstractValue)

function ReadGlass.__init__(self, ...)
	AbstractValue.__init__(self, ...)
	self.template  = "cbi/readglass"
end

--[[
HelpCommand - tell user what it is 
]]--
HelpCommand = class(AbstractValue)

function HelpCommand.__init__(self, ...)
	AbstractValue.__init__(self, ...)
	self.template  = "cbi/helpcommand"
end
--[[
EditBox - edit  
]]--
EditBox = class(AbstractValue)

function EditBox.__init__(self, ...)
	AbstractValue.__init__(self, ...)
	self.template  = "cbi/editbox"
end

--[[
TrashBin - delete 
]]--
TrashBin = class(AbstractValue)

function TrashBin.__init__(self, ...)
	AbstractValue.__init__(self, ...)
	self.template  = "cbi/trashbin"
end
